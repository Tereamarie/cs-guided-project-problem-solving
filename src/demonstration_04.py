...
- iterate over the hash tables items exporting the key and value of each item
...
def emotify(txt):
    ...
    # # iterate over the hash tables items extracting the key and value
    # for k, v in data.items():
    #     # set an updated string using the `.replace` method 
    #     # to return each txt with the new substring substituted for the old one
    #     txt = txt.replace(k, v)
    ...
    end_of_string = txt[8:]
    # concatonate start_of_string with value from data using end_of_string -> "smile" ->  ":)" as the key 
    # and set them to full_string
